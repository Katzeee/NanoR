...
class RHI {
 public:
  ...
  virtual bool BindVertexBuffer(const RHIBindVertexBufferInfo &bind_vertex_buffer_info,
                                ...
                                std::shared_ptr<RHIBuffer> vertex_buffer) = 0;
  virtual bool BindIndexBuffer(const RHIBindIndexBufferInfo &bind_index_buffer_info,
                               std::shared_ptr<RHIVertexArray> vertex_array,
                               std::shared_ptr<RHIBuffer> index_buffer) = 0;
  virtual bool CreateShaderModule(const RHIShaderModuleCreateInfo &shader_create_module_info,
                                  std::shared_ptr<RHIShaderModule> &shader_module) = 0;
  virtual bool CreateShaderProgram(const RHIShaderProgramCreateInfo &shader_program_create_info,
                                   std::shared_ptr<RHIShaderProgram> &shader_program) = 0;
