...
class Shader {
 public:
  ...
  void AddDefine(const std::string &group, const std::string &name);
  void DeleteDefine(const std::string &group, const std::string &name);
  void ClearDefineGroup(const std::string &group);
  void CompileShaders();
  [[nodiscard]] auto GetId() const -> const unsigned int & {
    return id_;
  }
 ...
 private:
  ...
  std::string vs_str_;
  std::string fs_str_;
  std::map<std::string, std::set<std::string>> define_consts_;
};
