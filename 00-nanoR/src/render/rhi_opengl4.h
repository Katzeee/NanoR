...
public:
  ...
  void DrawIndexed(const std::shared_ptr<RHIBuffer> &vertex_buffer, const std::shared_ptr<RHIBuffer> &index_buffer,
      RHIShaderProgram const *shader_program, std::optional<RHIFramebuffer const *> framebuffer) override;
  ...
  auto BindVertexBuffer(const RHIBindVertexBufferInfo &bind_vertex_buffer_info, std::shared_ptr<RHIVertexArray> vertex_array, std::shared_ptr<RHIBuffer> vertex_buffer) -> bool override;
  auto BindIndexBuffer(const RHIBindIndexBufferInfo &bind_index_buffer_info, std::shared_ptr<RHIVertexArray> vertex_array, std::shared_ptr<RHIBuffer> index_buffer) -> bool override;
